...
const SENSITIVE_PATTERNS: SensitivePattern[] = [
	...
	{
		name: "bearer-token",
		pattern: /Bearer\s+[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)*/g,
		mask: "[BEARER_TOKEN_MASKED]",
	},
	{
		name: "jwt-token",
	...
	{
		name: "access-token",
		pattern: /access[_-]?token[:\s=]+['"]?[a-zA-Z0-9_-]{8,}['"]?/gi,
		mask: "[ACCESS_TOKEN_MASKED]",
	},
	{
		name: "refresh-token",
		pattern: /refresh[_-]?token[:\s=]+['"]?[a-zA-Z0-9_-]{8,}['"]?/gi,
		mask: "[REFRESH_TOKEN_MASKED]",
	},
	...
	// Firebase config with API key
	{
		...
		pattern: /apiKey[:\s]+['"][a-zA-Z0-9_-]{10,}['"]/gi,
		mask: "apiKey: [FIREBASE_API_KEY_MASKED]",
	},

	// Service account keys
	{
		name: "service-account",
		pattern: /"private_key_id"\s*:\s*"[^"]+"/g,
		mask: '"private_key_id": "[MASKED]"',
	},
	{
		name: "client-secret",
		pattern: /client[_-]?secret[:\s=]+['"]?[a-zA-Z0-9_-]{8,}['"]?/gi,
		mask: "[CLIENT_SECRET_MASKED]",
	},
];
...
function toSafeString(value: unknown): string {
	...
	if (value instanceof Error)
		return sanitizeMessage(`${value.name}: ${value.message}`);
...
export function validateSafeToLog(message: string): boolean {
	for (const { pattern, name } of SENSITIVE_PATTERNS) {
		if (pattern.test(message)) {
			console.warn(
				`[SECURITY WARNING] Attempted to log message containing potential ${name}. Use secureLog instead.`,
			);
			return false;
		}
	}
...
export const secureLog = {
	debug: (message: unknown, ...args: unknown[]) =>
		log("debug", toSafeString(message), ...args),
	info: (message: unknown, ...args: unknown[]) =>
		log("info", toSafeString(message), ...args),
	warn: (message: unknown, ...args: unknown[]) =>
		log("warn", toSafeString(message), ...args),
	error: (message: unknown, ...args: unknown[]) =>
		log("error", toSafeString(message), ...args),
